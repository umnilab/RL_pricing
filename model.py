...
class TwinCriticCNN(torch.nn.Module):
    ...
    def forward(self, x1d: torch.Tensor, x: torch.Tensor, a: torch.Tensor, t: torch.Tensor, /) -> torch.Tensor:
        ...
        if self.n <= 0:
            q2 = torch.relu(self.linears2[0](q2) + self.positional_embedding(t.type(torch.long)).squeeze(1))
        else:
            q2 = torch.concat([q2, Variable(self.pe[t[:, 0]], requires_grad=False)], 1)
            q2 = torch.relu(self.linears2[0](q2))
...
"""SAC MLP"""
# Get the pi(a_t, s_t)
# Get the update equations
class SACCriticMLP(torch.nn.Module):
    def __init__(
            self,
            size: int,
            max_waiting: int,
        ...
        super(SACCriticMLP, self).__init__()
        buf = []
        buf2 = []
        shapes_ = [size * size * max_waiting + size * (max_traveling + max_duration) + size + size + 1 + n] + shapes
        shapes2 = [size * size * max_waiting + size * (max_traveling + max_duration) + size + 1 + n] + shapes
        for (num_ins, num_outs) in zip(shapes_[:-1], shapes_[1:]):
            buf.append(torch.nn.Linear(num_ins, num_outs))
        for (num_ins, num_outs) in zip(shapes2[:-1], shapes2[1:]):
            buf2.append(torch.nn.Linear(num_ins, num_outs))
        self.linears = torch.nn.ModuleList(buf)
        self.linears2 = torch.nn.ModuleList(buf2)
        self.max_len = max_len
    ...
    def forward(self, x: torch.Tensor, x2d: torch.Tensor, a: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        s = torch.flatten(x, start_dim=1)
        x = torch.concat([s, a], 1)
        t = t % self.max_len
        if self.n <= 0:
            q = torch.relu(self.linears[0](x) + self.positional_embedding(t.type(torch.long)).squeeze(1))
            v = torch.relu(self.linears2[0](s) + self.positional_embedding2(t.type(torch.long)).squeeze(1))
        else:
            s = torch.concat([s, Variable(self.pe[t[:, 0]], requires_grad=False)], 1)
            x = torch.concat([x, Variable(self.pe[t[:, 0]], requires_grad=False)], 1)
            q = torch.relu(self.linears[0](x))
            v = torch.relu(self.linears2[0](s))
        for (l, linear) in enumerate(self.linears[1:-1]):
            #
            q = linear.forward(q)
            q = torch.relu(q)
...
"""SAC CNN"""
class SACCriticCNN(torch.nn.Module):
    def __init__(
            self,
            size: int, size2: int, size3: int, size4: int, channels: List[int], shapes: List[int],
            kernel_size_conv: int, stride_size_conv: int, kernel_size_pool: int,
        ...
        super(SACCriticCNN, self).__init__()
        ...
        self.pool = torch.nn.AvgPool2d(kernel_size=kernel_size_pool, stride=stride_size_pool)
        buf = []
        shapes2 = [size * size2 * channels[-1] + size3 + size4 + 1 + n] + shapes  # add action in the fully connected layer
        for (num_ins, num_outs) in zip(shapes2[:-1], shapes2[1:]):
            #
            buf.append(torch.nn.Linear(num_ins, num_outs))
        self.linears = torch.nn.ModuleList(buf)
        ...
        self.pool2 = torch.nn.AvgPool2d(kernel_size=kernel_size_pool, stride=stride_size_pool)
        buf2 = []
        shapes3 = [size * size2 * channels[-1] + size3 + 1 + n] + shapes  # do not add action in the fully connected layer
        for (num_ins, num_outs) in zip(shapes3[:-1], shapes3[1:]):
            buf2.append(torch.nn.Linear(num_ins, num_outs))
        self.linears2 = torch.nn.ModuleList(buf2)
        self.max_len = max_len
    ...
    def forward(self, x1d: torch.Tensor, x: torch.Tensor, a: torch.Tensor, t: torch.Tensor, /) -> torch.Tensor:
        ...
        q = torch.concat([q, x1d, a], 1)
        if self.n <= 0:
            q = torch.relu(self.linears[0](q) + self.positional_embedding(t.type(torch.long)).squeeze(1))
        else:
            q = torch.concat([q, Variable(self.pe[t[:, 0]], requires_grad=False)], 1)
            q = torch.relu(self.linears[0](q))
        ...
        v = torch.concat([v, x1d], 1)
        if self.n <= 0:
            v = torch.relu(self.linears2[0](v) + self.positional_embedding(t.type(torch.long)).squeeze(1))
        else:
            v = torch.concat([v, Variable(self.pe[t[:, 0]], requires_grad=False)], 1)
            v = torch.relu(self.linears2[0](v))

        for linear in self.linears2[1:-1]:
            v = torch.relu(linear(v))
        v = self.linears2[-1](v)
